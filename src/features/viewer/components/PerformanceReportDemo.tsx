...
export default function PerformanceReportDemo() {
  ...
  const mockStats = {
    ...
    recommendations: [
      ...
      'Optimize texture sizes to reduce memory usage'
    ]
  }
        ...
        <div className="space-y-6">
          ...
          <div className="bg-zinc-900/50 rounded-lg p-4 border border-zinc-800">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className={`text-2xl font-bold ${getScoreColor(mockStats.optimizationScore)}`}>
                {mockStats.optimizationScore}/100
              </span>
            </div>
            <Progress value={mockStats.optimizationScore} className="mb-2" />
            <div className="flex items-center gap-2">
              <Badge className={getComplexityColor(mockStats.complexity)}>
                {mockStats.complexity} Complexity
              </Badge>
              <span className="text-sm text-zinc-400">
                Load time: {mockStats.loadTime}ms
              </span>
            </div>
          </div>
          ...
          <div className="bg-zinc-900/50 rounded-lg p-4 border border-zinc-800">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <div className="text-2xl font-bold text-white">{mockStats.triangles.toLocaleString()}</div>
                <div className="text-sm text-zinc-400">Triangles</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-white">{mockStats.vertices.toLocaleString()}</div>
                <div className="text-sm text-zinc-400">Vertices</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-white">{mockStats.meshes}</div>
                <div className="text-sm text-zinc-400">Meshes</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-white">{mockStats.drawCalls}</div>
          ...
          <div className="bg-zinc-900/50 rounded-lg p-4 border border-zinc-800">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <div className="text-2xl font-bold text-white">{mockStats.materials}</div>
                <div className="text-sm text-zinc-400">Materials</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-white">{mockStats.textures}</div>
                <div className="text-sm text-zinc-400">Textures</div>
              </div>
              <div className="col-span-2">
                <div className="text-2xl font-bold text-white">{mockStats.textureMemory} MB</div>
          ...
          <div className="bg-zinc-900/50 rounded-lg p-4 border border-zinc-800">
            ...
            <div className="space-y-2">
              {mockStats.recommendations.map((rec, index) => (
